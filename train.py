...
try:
    # new timm import
    from timm.layers import convert_sync_batchnorm, set_layer_config
except ImportError:
...
def main(args=None):
    ...
    args, args_text = _parse_args(args)
    ...
    if args.torchscript:
        assert (
            not args.torchcompile
        ), "Cannot use torch.compile() with torch.jit.script()"
        assert (
            not use_amp == "apex"
